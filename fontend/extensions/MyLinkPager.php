...
class MyLinkPager extends CLinkPager
{
        ...
        protected function createPageButtons()
        {
               ...
               // if($currentPage <=1)
                ...
                if(($page=$currentPage-1)<0)
                        $page=0;
                if($currentPage >= 1)      
                  $buttons[]=$this->createPageButton('<i class="fa fa-chevron-left"></i>',$page,self::CSS_PREVIOUS_PAGE,$currentPage<=0,false);
                ...
                if($currentPage < $pageCount-1)
                  $buttons[]=$this->createPageButton('<i class="fa fa-chevron-right"></i>',$page,self::CSS_NEXT_PAGE,$currentPage>=$pageCount-1,false);
  ...
  /**
        ...
        protected function createPageButtonajax($label,$page,$class,$hidden,$selected)
    	{
    		if($hidden || $selected)
    			$class.=' '.($hidden ? self::CSS_HIDDEN_PAGE : self::CSS_SELECTED_PAGE);
    		return '<li class="'.$class.'">'.CHtml::link($label,$this->createPageUrlajax($page)).'</li>';
    	}
}
